...
def compute_kl_divergence(
    ...
    """
    Compute the KL divergence between the empirical distribution of the input
    samples and an isotropic standard Gaussian distribution using Pytorch.

    Parameters:
    samples (torch.Tensor): A 2D tensor with rows as samples and columns as features.
...
def eval_causal_structure_binary(
    a_true: np.ndarray,
    a_pred: np.ndarray,
    diagonal: bool = False,
) -> Tuple[float, float]:
    if not diagonal:
        ...
        precision = precision_score(
            y_true=a_true_offdiag,
            y_pred=a_pred_offdiag,
        )
        recall = recall_score(
            y_true=a_true_offdiag,
            y_pred=a_pred_offdiag,
        ...
        bal_arruracy = balanced_accuracy_score(
            y_true=a_true_offdiag,
            y_pred=a_pred_offdiag,
        )
        hamming_dist = np.sum(
            np.abs(a_true.flatten() - a_pred.flatten())
        ) / len(a_true.flatten())
    else:
        precision = precision_score(
    ...
    return accuracy, bal_arruracy, precision, recall, hamming_dist


def eval_causal_structure(
    a_true: np.ndarray,
    a_pred: np.ndarray,
    diagonal: bool = False,
    ...
    if not diagonal:
        ...
        if np.max(a_true_offdiag) == np.min(a_true_offdiag):
            ...
            auroc = roc_auc_score(
                y_true=a_true_offdiag.flatten(),
                y_score=a_pred_offdiag.flatten(),
            )
...
def construct_training_dataset(data: Any, order: int) -> Tuple[Array, Array]:
...
def pot(
    ...
    References:
    Siffer, Alban, et al. "Anomaly Detection is Streams with Extreme Value Theroy."
