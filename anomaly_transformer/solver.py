...
class EarlyStopping:
    ...
    def __call__(
        ...
        if self.best_score1 is None:
            ...
            self.save_checkpoint(val_loss1, val_loss2, model, path)
        elif (
            score1 < self.best_score1 + self.delta
    ...
    def save_checkpoint(
        ...
        torch.save(
            ...
            f=os.path.join(path, str(self.dataset) + '_checkpoint.pth')
        )
        self.val_loss1_min = val_loss1
...
class Solver(object):
    ...
    def validation(self, val_loader) -> Tuple[float, float]:
        self.model.eval()

        loss1 = []
        loss2 = []

        for _, (input_data, _) in enumerate(val_loader):
    ...
    def train(self) -> None:
        ...
        for epoch in range(self.num_epochs):
            ...
            val_loss1, val_loss2 = self.validation(self.test_loader)
    ...
    def test(self):
        self.model.load_state_dict(
            torch.load(
                os.path.join(
                    str(self.model_save_path), str(self.dataset)
                    + '_checkpoint.pth'
                )
            )
        )
