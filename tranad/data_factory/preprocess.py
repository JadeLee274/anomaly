...
def load_and_save(
    ...
    np.save(
        file=os.path.join(OUTPUT_FOLDER, f'SMD/{dataset}_{category}.npy'),
        arr=temp,
    )
    return temp.shape
...
def load_and_save2(
    ...
    with open(
        os.path.join(dataset_folder, 'interpretation_label', filename), 'r'
    ) as f:
        ls = f.readlines()
    
    for line in ls:
        pos, values = line.split(':')[0], line.split(':')[1].split(',')
        start, end, index = int(pos.split('-')[0]), int(pos.split('-')[1]), [int(i) - 1 for i in values]
...
def convertNumpy(df: pd.DataFrame):
...
def load_data(dataset: str):
    ...
    if dataset == 'synthetic':
        ...
        split = 10000
        train = normalize(data.values[:, :split].reshape(split, -1))
        test = normalize(data.values[:, split:].reshape(split, -1))
        label = pd.read_csv(test_labels, header=None)
        label[0] -= split
        labels = np.zeros(test.shape)

        for i in range(label.shape[0]):
            point = label.values[i][0]
            labels[point - 30: point + 30, label.values[i][1:]] = 1
        
        test += labels * np.random.normal(0.75, 0.1, test.shape)

        for file in ['train', 'test', 'labels']:
            np.save(
                file=os.path.join(folder, f'{file}.npy'),
                arr=eval(file)
            )
        ...
        for filename in file_list:
            ...
            temp = np.genfromtxt(
                fname=os.path.join(dataset_folder, filename),
                dtype=np.float64,
                delimiter=',',
            )
            ...
            for file in ['train', 'test', 'labels']:
                np.save(
                    file=os.path.join(folder, f'{d_num}_{file}.npy'),
                    arr=eval(file)
                )
        ...
        for filename in file_list:
            ...
            for file in ['train', 'test', 'labels']:
                np.save(
                    file=os.path.join(folder, f'{fn}_{file}.npy'),
                    arr=eval(file)
                )
        ...
        train = pd.read_csv(
            os.path.join(dataset_folder, 'WADI_14days.csv'),
            skiprows=1000,
            nrows=2e5,
        )
        ...
        test = convertNumpy(test)
        labels = convertNumpy(labels)
...
if __name__ == '__main__':
    ...
    if len(commands) > 0:
        ...
        print('Usage: python preprocess.py <dataset>')
